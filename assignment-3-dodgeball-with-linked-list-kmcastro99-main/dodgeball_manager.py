...
class DodgeballManager:
    ...
    def __init__(self, throwers : [str], dodgers : [str]):
        ...
        if throwers == [] \
            ...
            or dodgers == [] \
            or dodgers == None:
            raise ValueError("throwers and dodgers must be non-empty Python lists")

        for thrower_name in throwers:
            self._throwerList.append(DodgeballPlayer(thrower_name))
        for dodger_name in dodgers:
            self._dodgerList.append(DodgeballPlayer(dodger_name))
